...
def count_letters(string):
    ...
    for letter in string:
        if not letter.isspace():
            # Look up letter in counts dictionary
            #   If letter is in count, use the existing value,
            #   OtherWise, if letter is not in count, use the default value of 0
...
def main():
    ...
    while arguments and arguments[0].startswith('-'):
        argument = arguments.pop(0)                     # Discuss: popping from queue = shift 
        if argument == '-i':
            ignorecase = True
        elif argument == '-h':
            usage(0)
